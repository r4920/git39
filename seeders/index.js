...
async function seedUser () {
  try {
    ...
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Tatum.Kertzmann' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
    ...
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Granville64' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
